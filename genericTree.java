...
public class genericTree {
    ...
    // static int does not set istself 0 when the method is called the second time ,
    // it still has the old intial value whihc you need to set at 0 through flag;
    ...
    public static int height(Node n) {
        ...
        ht++;
        // ht++ is very important here as it makes the the change in height for the
        // current node adding 1 to it.
        // therefore making the height of that node from the deepest leaf that it has to
        // offer accurate as
        // this +1 considers the edge between the parent node at that instance to the
        // child node at that instance.
        return ht;
    }
    ...
    public static void levelOrderTraversalLine(Node node) {
        ...
        while (!parent.isEmpty() || !child.isEmpty()) {
            ...
            if (parent.isEmpty()) {
                ...
                // the one above or is comparatively worse option as addAll has time compleixity
                // of O(k)
                // whereas parent=child makes it just address allocation which makes the
                // complexity as O(1)
            }
        }
    ...
    public static boolean removeLeafNode1(Node root) {
        ...
        // so before this we tried two apporaches first one was directly making chnages
        // in the root.children node
        // the mistake in that was if leaf node was removed then the iteration for the
        // next root would be skipped as it would move a index ahead.
        // then we tried ArrayList temp=root.children;
        // the same issue as here temp just refers to the same address as root.children
        // so we use clone so that temp refers to a new arraylist (copy of the original
        // one) but the changes in the original does not affect it
    ...
    public static void removeLeafNode2(Node root) {
        ...
        // the remove child part should come beofre otherwise it will also remove extra
        // nodes which were not leaf earlier..
        for (Node x : root.children) {
            removeLeafNode2(x);
        }
        // for(int i=root.children.size()-1;i>=0;i--){
    ...
    public static void linearize1(Node root) {
        ...
        while (root.children.size() > 1) {
            ...
            // this will linearize all the nodes except the root node where it wont give us
            // the structure we want.
        }
        // now everything before the root node has been linearized but to get the
        // structure we want we have to connect every tail of second level height
        // nodes to the first node of the second linearized branch
        // the only thing that has not been balanced here is rooot node so it can have
        // size>1
    }
    ...
    private static Node getTail(Node root) {
        ...
        return root;
        // System.out.println(root.data);
        // there is some fundamentma; mistake in the recursion happening here clear it
        // and add to notes
    }

    // the current approach is O(n2) but if you want the code to be O(N) something
    // has to chnage in the recursion so that it provides
    // the tail and linearizes both in the same backtracking recursion
    public static Node linearize2(Node node) {
        if (node.children.size() == 0) {
            return node;
        }
        Node lkt = linearize2(node.children.get(node.children.size() - 1));
        // the loop that linearizes and connects the solution
        while (node.children.size() > 1) {
            Node last = node.children.remove(node.children.size() - 1);
            Node sl = node.children.get(node.children.size() - 1);
            Node slkt = linearize2(sl);
            // where slkt becomes equal to lkt the tree starts to linearize itself and put
            // everything in place
    ...
    public static boolean findElement1(Node node, int val) {
        boolean result = false;
        if(val==node.data)result=true;
        for(Node x:node.children){
            boolean Lresult=findElement1(x,val);
            if(Lresult){
                result=true;
            }
        }
    ...
    public static boolean findElement2(Node node, int val) {
        if(val==node.data)return true;
        for(Node x:node.children){
            boolean Lresult=findElement2(x,val);
            if(Lresult){
                return true;
                // helps to skip the unecessary euler path
                // return truly means that to give back the resul to the function it was last called to
            }
        }
        return false;
    }
    public static ArrayList nodeToRootPath1(Node node,int val){
        ...
        for(Node x:node.children){
            ArrayList<Integer> Lsol=nodeToRootPath1(x, val);
            if(Lsol.size()>0){
                Lsol.add(node.data);
                return Lsol;
            }
...
// been provided any tree and for that
